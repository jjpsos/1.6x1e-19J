...
pub async fn index() -> impl Responder {
    ...
    context.insert("message_from_rust", "Hello from Jim!");
...
pub async fn login_post(data: web::Form<LoginForm>) -> impl Responder {
    let logged_in = auth::validate_login(&data.username, &data.password); // true
    if logged_in {
        let settings = settings::get_settings();

        /*
        let mut context = tera::Context::new();
        context.insert("error_message", "Login failed on Jim-Test");
        let page_content = TEMPLATES.render("login.html", &context).unwrap();
        HttpResponse::Ok().body(page_content)
        */
